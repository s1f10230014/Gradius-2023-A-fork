    ...
    var depend13 = function (dependencies, cb) {
      ...
      fn.inject = function (deps) {
        return typeof deps === 'function'
          ? exports.depend(__assign(__assign({}, dependencies), deps(dependencies)), cb)
          : exports.depend(__assign(__assign({}, dependencies), deps), cb);
      };
      return fn;
    };
...
var firebaseAdmin = import_firebase_admin.default.initializeApp(
  FIREBASE_AUTH_EMULATOR_HOST !== void 0
    ? { projectId: 'emulator' }
    : { credential: import_firebase_admin.default.credential.cert(JSON.parse(FIREBASE_SERVER_KEY)) }
);
...
var callParserIfExistsQuery = (parser) => (req, reply, done) =>
  Object.keys(req.query).length ? parser(req, reply, done) : done();
...
var server_default = (fastify, options2 = {}) => {
  ...
  fastify.get(`${basePath}/check`, asyncMethodToHandler(controller1.get));
  fastify.get(`${basePath}/health`, methodToHandler(controller2.get));
  fastify.get(`${basePath}/ip`, methodToHandler(controller3.get));
  fastify.get(
    `${basePath}/me`,
    {
  ...
  fastify.get(`${basePath}/rooms`, methodToHandler(controller5.get));
  fastify.post(`${basePath}/rooms/control`, asyncMethodToHandler(controller6.post));
  fastify.get(`${basePath}/rooms/gunPosition`, methodToHandler(controller7.get));
  fastify.post(`${basePath}/rooms/gunPosition`, asyncMethodToHandler(controller7.post));
  fastify.get(`${basePath}/rooms/playerPosition`, methodToHandler(controller8.get));
  fastify.post(
    `${basePath}/session`,
    {
  ...
  fastify.post(`${basePath}/tasks`, asyncMethodToHandler(controller10.post));
  fastify.patch(
